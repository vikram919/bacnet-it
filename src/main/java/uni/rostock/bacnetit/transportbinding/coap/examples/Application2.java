		...
		ChannelListener bacnetDevice2001 = new ChannelListener(new BACnetEID(2001)) {
			...
			public void onIndication(T_UnitDataIndication arg0, Object context) {
				...
				if (incoming instanceof ConfirmedRequest
						&& ((ConfirmedRequest) incoming).getServiceRequest() instanceof ReadPropertyRequest) {
					...
					new ReadPropertyAck(new BACnetObjectIdentifier(BACnetObjectType.analogValue, 1),
							BACnetPropertyIdentifier.presentValue, new UnsignedInteger(1), new Real(value))
									.write(byteQueue);

					// Send answer
					try {
						System.out.println("Application2 sends an ReadPropertyAck to Application1");
						sendBACnetMessage(new URI("coap://localhost:8080/transport"), new BACnetEID(2001), new BACnetEID(1001),
								byteQueue.popAll());
						...
						&& ((UnconfirmedRequest) incoming).getService() instanceof IAmRequest) {
					System.out.println("Application2 got an indication - IAmRequest ");
				}
