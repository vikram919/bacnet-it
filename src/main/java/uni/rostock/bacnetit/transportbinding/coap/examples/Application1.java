...
public class Application1 extends AbstractApplication {
	...
	public Application1(ApplicationService applicationService) {
		...
		// to get implemented.
		ChannelListener bacnetDevice1001 = new ChannelListener(new BACnetEID(1001)) {
			 ...
			 */
			@Override
			public void onIndication(T_UnitDataIndication arg0, Object context) {
				...
				if (incoming instanceof ComplexACK) {
					System.out.println("Application1 got an indication - ReadPropertyAck");
					System.out.println("************\nReceived Value: "
							+ ((ComplexACK) incoming).getService().toString().split("\\(")[1].split("\\)")[0]
							+ "\n************");
				}
				// Dummy Handling of a WhoIsRequest
					...
					byte[] iAmRequest = new byte[] { (byte) 0x1E, (byte) 0x0E, (byte) 0xC4, (byte) 0x02, (byte) 0x00,
							(byte) 0x00, (byte) 0x00, (byte) 0x21, (byte) 0x01, (byte) 0x91, (byte) 0x00, (byte) 0x21,
							(byte) 0x01, (byte) 0x0F, (byte) 0x1F };

					try {
						System.out.println("Application1 sends an IAmRequest to Application2");
						sendBACnetMessage(new URI("coap://localhost:9090/transport"), new BACnetEID(1001), new BACnetEID(2001),
								iAmRequest);
					} catch (Exception e) {
					}
		...
		// To simulate a device within the application, a ChannelListener needs
		// to get implemented.
		ChannelListener bacnetDevice1002 = new ChannelListener(new BACnetEID(1002)) {
