...
public class TestLocalSpecService {
	...
	private static URI hostLocal1Tls = null, hostLocal1NoTls = null, hostLocal2Tls = null, hostLocal2NoTls = null;
	private static int portLocal1Tls = 0, portLocal1NoTls = 0, portLocal2Tls = 0, portLocal2NoTls = 0;
	...
	final KeystoreConfig keystoreConfig = new KeystoreConfig("dummyKeystores/keyStoreDev1.jks", "123456",
			"operationaldevcert");
	final TruststoreConfig truststoreConfig = new TruststoreConfig("dummyKeystores/trustStore.jks", "123456",
			"installer.ch");
	...
	private void setupHost1() throws URISyntaxException {
		...
		connectionFactory.addConnectionServer("wss",
				new WSSConnectionServerFactory(portLocal1Tls, keystoreConfig, truststoreConfig));

		// TLS off, Client
		connectionFactory.addConnectionClient("ws", new WSConnectionClientFactory());
		// TLS off, Server
		connectionFactory.addConnectionServer("ws", new WSConnectionServerFactory(portLocal1NoTls));
		...
		/*
		 * Implement and set BACnetEntityHandler (to handle received control messages)
		 */
		...
		final NetworkPortObj npo = new NetworkPortObj("wss", 8001, keystoreConfig);

		/*
		 * Channel Listener is the interface between the stack and the application. A
		 * simple BACnet Device gets simulated by an anonymous class implementing
		 * ChannelListener. Add two devices to each host and notify the monitor.
		 */
		((ChannelConfiguration) appService1).registerChannelListener(new ChannelListener(devLocal11) {

			@Override
			public void onIndication(final T_UnitDataIndication tUnitDataIndication, final Object ctx) {
				indicationTodevLocal11 = tUnitDataIndication;
			}

			@Override
			public void onError(T_ReportIndication tReportIndication, String cause) {
				System.err.println(tReportIndication);
			}
	...
	private void setupHost2() throws URISyntaxException {
		...
		// TLS on, Client
		connectionFactory2.addConnectionClient("wss", new WSSConnectionClientFactory(keystoreConfig, truststoreConfig));
		// TLS on, Server
		connectionFactory2.addConnectionServer("wss",
				new WSSConnectionServerFactory(portLocal2Tls, keystoreConfig, truststoreConfig));

		// TLS off, Client
		connectionFactory2.addConnectionClient("ws", new WSConnectionClientFactory());
		// TLS off, Server
		connectionFactory2.addConnectionServer("ws", new WSConnectionServerFactory(portLocal2NoTls));
		...
		final NetworkPortObj npo2 = new NetworkPortObj("wss", 9001, keystoreConfig);
	...
	private void setupDiscovery() {
		ds = new DiscoveryConfig("DNSSD", "86.119.39.127", "itb.bacnet.ch.", "bds._sub._bacnet._tcp.",
	...
	public void TestSubscribeCOVRequest() {
		final SubscribeCOVRequest scr = new SubscribeCOVRequest(new UnsignedInteger(13),
				new BACnetObjectIdentifier(BACnetObjectType.binaryInput, 1), new Boolean(true),
		...
		assertEquals(Arrays.toString(before), Arrays.toString(indicationTodevLocal21.getData().getBody()));
	}

	@Test
	public void TestSubscribeCOVPropertyRequest() {
		final SubscribeCOVPropertyRequest scpr = new SubscribeCOVPropertyRequest(new UnsignedInteger(10),
				new BACnetObjectIdentifier(BACnetObjectType.notificationClass, 1), new Boolean(false),
				new UnsignedInteger(14),
				new PropertyReference(BACnetPropertyIdentifier.activeText, new UnsignedInteger(99)), new Real(30));
		final ByteQueue b = new ByteQueue();
		scpr.write(b);
		final TPDU tpdu = new TPDU(devLocal11, devLocal21, b.popAll());
		final T_UnitDataRequest dDataRequest = new T_UnitDataRequest(hostLocal2NoTls, tpdu, 1, null);
	...
	public void TestLifeSafetyOperationRequest() {
		final LifeSafetyOperationRequest lsor = new LifeSafetyOperationRequest(new UnsignedInteger(10),
				new CharacterString("abc"), LifeSafetyOperation.unsilenceAudible,
	...
	public void TestConfirmedCovNotificationRequest() {
		...
		seq.add(new PropertyValue(BACnetPropertyIdentifier.activeText, new UnsignedInteger(10)));
		seq.add(new PropertyValue(BACnetPropertyIdentifier.deviceAddressBinding, new UnsignedInteger(17)));
		final ConfirmedCovNotificationRequest ccnr = new ConfirmedCovNotificationRequest(new UnsignedInteger(17),
				new BACnetObjectIdentifier(BACnetObjectType.binaryInput, 1),
				new BACnetObjectIdentifier(BACnetObjectType.notificationClass, 1), new UnsignedInteger(70), seq
	...
	public void TestConfirmedEventNotificationRequest() {
		final ConfirmedEventNotificationRequest cenr = new ConfirmedEventNotificationRequest(new UnsignedInteger(77),
				new BACnetObjectIdentifier(BACnetObjectType.analogValue, 1),
				new BACnetObjectIdentifier(BACnetObjectType.averaging, 1), new TimeStamp(new DateTime()),
				new UnsignedInteger(7), new UnsignedInteger(11), EventType.unsignedRange,
				new CharacterString("test123"), NotifyType.ackNotification, new Boolean(true), EventState.highLimit,
				EventState.lowLimit, null
	...
	public void TestAcknowledgeAlarmRequest() {
		...
		final AcknowledgeAlarmRequest aar = new AcknowledgeAlarmRequest(new UnsignedInteger(1),
				new BACnetObjectIdentifier(BACnetObjectType.analogValue, 1), EventState.highLimit,
				new TimeStamp(new DateTime()), new CharacterString("test"), new TimeStamp(new DateTime()));
		final ByteQueue b = new ByteQueue();
		aar.write(b);
		final TPDU tpdu = new TPDU(devLocal11, devLocal21, b.popAll());
		final T_UnitDataRequest dDataRequest = new T_UnitDataRequest(hostLocal2NoTls, tpdu, 1, null);
	...
	public void TestReadPropertyRequest() {
		...
		assertEquals(Arrays.toString(before), Arrays.toString(indicationTodevLocal12.getData().getBody()));
	}
	...
	public void TestWritePropertyRequest() {
		final WritePropertyRequest asdu = new WritePropertyRequest(
				new BACnetObjectIdentifier(BACnetObjectType.analogValue, 1), BACnetPropertyIdentifier.presentValue,
				new UnsignedInteger(55), new Real(5), new UnsignedInteger(1));
		final ByteQueue b = new ByteQueue();
		asdu.write(b);
		...
		assertEquals(Arrays.toString(before), Arrays.toString(indicationTodevLocal11.getData().getBody()));
